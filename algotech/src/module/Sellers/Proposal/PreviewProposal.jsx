            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p><strong>Endereço:</strong> {endereco}, {bairro}, {cidade} - {uf_cidade}</p>
                        <p><strong>CEP:</strong> {cep}</p>
                    </div>
                    <div>
                        <p><strong>Nome da Mãe:</strong> {nome_mae}</p>
                        <p><strong>Nome do Pai:</strong> {nome_pai}</p>
                        <p><strong>RG:</strong> {rg_documento}</p>
                        <p><strong>Órgão Emissor:</strong> {orgao_emissor}</p>
                        <p><strong>Data de Emissão:</strong> {data_emissao}</p>
                        <p><strong>Naturalidade:</strong> {naturalidade} - {uf_naturalidade}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p><strong>Tipo de Benefício:</strong> {tipo_beneficio}</p>
                        <p><strong>Matrícula:</strong> {matricula}</p>
                        <p><strong>Salário Líquido:</strong> R$ {salario_liquido}</p>
                        <p><strong>Valor da Operação:</strong> R$ {valor_operacao}</p>
                        <p><strong>Margem:</strong> R$ {margem}</p>
                        <p><strong>Prazos:</strong> {prazo_inicio} a {prazo_fim}</p>
                    </div>
                    <div>
                        <p><strong>Tipo de Operação:</strong> {tipo_operacao}</p>
                        <p><strong>Tipo de Pagamento:</strong> {tipo_pagamento}</p>
                        <p><strong>Banco:</strong> {banker_name}</p>
                        <p><strong>Agência:</strong> {agencia_banco}</p>
                        <p><strong>Número da Conta:</strong> {numero_conta}</p>
                        <p><strong>Tabela:</strong> {nome_tabela}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(image_urls).map(([key, urls]) => (
                        urls.length > 0 && (
                            <div key={key}>
                                <h3 className="font-semibold">{key.replace(/_/g, ' ').toUpperCase()}</h3>
                                {urls.map((url, index) => (
                                    <div key={index} className="mt-2">
                                        <img src={url} alt={`${key}_${index}`} className="w-full h-auto rounded-lg shadow-sm" />
                                    </div>
                                ))}
                            </div>
                        )
